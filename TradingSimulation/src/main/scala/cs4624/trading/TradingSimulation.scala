...
object TradingSimulation extends App {
  ...
  SentimentAnalysisModel.load("../cs4624_sentiment_analysis_model") match {
    case Some(sentimentAnalysisModel) =>
      val testStrategy = new TestStrategy(symbols, sentimentAnalysisModel, hBaseStockPriceDataSource, hbaseTweetDataSource, 100000, "results.csv")
      ...
      context.run {
        case (portfolio, MarketOpen(time)) =>
          printWriter.println("\"" + time + "\"," + portfolio.portfolioValue)
          printWriter.flush()
        case _ =>
      }
      printWriter.close()
