...
object APITester extends App {
  ...
  val symbols = Seq("^GSPC")// Seq("AAPL", "FB", "GILD", "KNDI", "MNKD", "NQ", "PLUG", "QQQ", "SPY", "TSLA", "VRNG")
  val future = Future.sequence(symbols.map { sym =>
    api.getQuotes(symbol = sym, startDate = LocalDate.of(2014, 1, 2), endDate = LocalDate.of(2014, 12, 31))
  })
